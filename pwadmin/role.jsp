...
<%
	...
	if(request.getParameter("show") != null && request.getParameter("show").compareTo("details") == 0)
	{
		...
		try
		{
						...
						else
						{
							...
							else
							{
								if(GameDB.renameRole(id, character.base.name.getString(), newname) == 0)
								{
									...
									UniquenameDB.PostDeleteRole(0, 0, id, character.base.name.getString());
			...
			if(character.base.spouse != 0)
			{
				try
				{
					spouse = StringEscapeUtils.escapeHtml(GameDB.get(character.base.spouse).base.name.getString());
				}
				catch(Exception e)
				{
					spouse = "INVALID";
								...
								while(rs.next())
								{
									...
									if(dv != null)
									{
										...
										while(itr.hasNext())
										{
											...
											String rolename = StringEscapeUtils.escapeJava(StringEscapeUtils.escapeHtml(chr.base.name.getString()));
